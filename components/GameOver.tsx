...
export function GameOver({
  ...
  useEffect(() => {
    ...
    fetch(`/api/leaderboard?check=${leaderboardScore}`, {
      ...
      .then((r) => r.json())
      .then((data) => {
        if (data.eligible !== false) setModalOpen(true);
      })
      .catch((err) => {
        if (err.name !== "AbortError") setModalOpen(true);
      });
      ...
      <div
        ...
        aria-label={
          isVictory
            ...
            : `Game over. ${score} risposte corrette.`
        }
      >
        ...
        <p className={styles.message}>
          {isVictory ? (
            ...
            <>
              <strong>{score}</strong>{" "}
              {score === 1 ? "risposta corretta" : "risposte corrette"}.
            </>
          )}
        </p>
          ...
          <div className={styles.breakdown}>
            ...
            <div className={styles.row}>
              ...
              <span className={styles.rowValue}>
                ...
                {avgTimeSec && (
                  <span className={styles.rowTime}> ({avgTimeSec}s)</span>
                )}
              </span>
            </div>
          ...
          <div className={styles.modal}>
            ...
            {submitState === "success" ? (
              <div className={styles.saved}>
                <p>
                  <strong>{savedNickname}</strong> salvato in classifica!
                </p>
                <a
                  href="/leaderboard"
                  className={`${btnStyles.btn} ${btnStyles.primary}`}
