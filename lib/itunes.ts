...
function isUsableTrack(t: iTunesTrack, searchedArtist: string): boolean {
  ...
  // Verifica artista: normalizza solo ASCII (ignora coreano, punteggiatura, emoji).
  // Nomi corti (≤5 char): match ESATTO — previene "paw"→"pawpatrol", "brad"→"bradpaisley"
  // Nomi lunghi: substring match bidirezionale (logica originale)
