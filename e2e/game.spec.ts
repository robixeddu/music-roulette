...
test.describe("Song Roulette - Flusso principale", () => {
  test("la homepage si carica e mostra le regole", async ({ page }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: /Song Roulette/i })
    ).toBeVisible();
    await expect(page.getByRole("list")).toBeVisible();
    await expect(
      page.getByRole("link", { name: /Inizia a giocare/i })
    ).toBeVisible();
  });
  ...
  test("la pagina di gioco mostra il pulsante di start", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Inizia a giocare/i })
    ).toBeVisible();
  });
  ...
  test("cliccando start appare il player audio", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Riproduci estratto/i })
    ).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test("il skip link porta al contenuto principale", async ({ page }) => {
    ...
    const skipLink = page.getByRole("link", {
      name: /Vai al contenuto principale/i,
    });
    await expect(skipLink).toBeFocused();
  });

  test("la navigazione da tastiera funziona sulle opzioni", async ({
    page,
  }) => {
    await page.goto("/game");
    await page.getByRole("button", { name: /Inizia a giocare/i }).click();
